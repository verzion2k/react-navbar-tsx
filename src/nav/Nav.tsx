...
export default class Nav extends Component<Props, State> {
  ...
  handleDropdown = (e: MouseEvent<Element>) => {
	...
	if (!items[id].selected && selectedItems.length > 0) 
	{
	  selectedItems.map(item => 
		...
		if (item.dropdownItems) 
		{
          item.dropdownItems.map(subItem =>
            subItem.selected ? (subItem.selected = false) : subItem.selected
		 );
	...
	else if (items[id].selected && selectedItems.length === 1) 
	{
		selectedItems.map(item => 
			{
          ...
          <main className="nav">
            <header className="nav__header">
              ...
              <div className="nav__right" onClick={this.handleOnToggle}>
                <FontAwesomeIcon
                  icon={faBars}
                  className="nav__collapse__button"
                />
              </div>
            </header>
            {toggleMenu && isMobile && (
